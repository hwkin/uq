...
def make_log_prob_fn(model, X, y, noise_std=0.01, prior_std=1.0):
    """
    Create log probability function for HMC sampling (Full Model).
    ...
    def _flat_to_param_dict(flat_params: torch.Tensor) -> OrderedDict:
        ...
        for name, p in named_params:
            numel = p.numel()
            # Ensure proper view and contiguous memory if necessary
...
def make_minibatch_log_prob_fn(model, X, y, batch_size=100, noise_std=0.01, prior_std=1.0):
    ...
    def log_prob(flat_params):
        ...
        # Comput predictions using functional call
        pred = torch.func.functional_call(model, params, (X_batch,))
...
def uqevaluation(num_test, test_data, model, method, hmc_samples=None, la_samples=None,sgld_samples=None, model_ensemble=None):
    ...
    if method == 'hmc':
        ...
        preds_eval_list = []
        with torch.no_grad():
            for paths in model_ensemble:  # model is a list of ensemble members
